...
for i=1:(size(FerryDistance,1)-1)
    noFerryUsedIndex=FerryDistance(i+1,:)<10;
...
beta_names_fix.(mode_choice_names{4})={'air_ASC','air_female'};  
X_names_fix.(mode_choice_names{4})={'ASC','female'};  

beta_names_fix.(mode_choice_names{5})={'ferry_ASC','ferry_VILLA'};  
X_names_fix.(mode_choice_names{5})={'ASC','VILLA'};  
...
% i need to check why air time beta is positive..
% i select the air obs, check their origin and destination, 
RVU_bortavaror_1_air=RVU_bortavaror_1(RVU_bortavaror_1.Mode==4,:);

origin=RVU_bortavaror_1_air.D_A_TransCadID;
destination=RVU_bortavaror_1_air.D_B_TransCadID;
[uniqueIDs,IA,IC]=unique(destination);
originIDs=airTime(2:end,1);
destinationIDs=airTime(1,2:end);
airTimeMatrix=airTime(2:end,2:end);
RVU_bortavaror_1_air.percentageTime=zeros(length(origin),1);
for i=1:length(origin)
    airTimeDestinationAlternatives=airTimeMatrix(ismember(originIDs,origin(i)),:);
    airTimeDestination=airTimeMatrix(ismember(originIDs,origin(i)),ismember(destinationIDs,destination(i)));
    RVU_bortavaror_1_air.percentageTime(i)=find(sort(airTimeDestinationAlternatives)==airTimeDestination)/length(airTimeDestinationAlternatives);
...
for i=1:length(uniqueIDs)
    uniqueIDs(i,2)=sum(ismember(destination,uniqueIDs(i)));
end
