...
RVU_bortavaror_air=RVU_bortavaror(RVU_bortavaror.Mode==4,:);

origin=RVU_bortavaror_air.D_A_TransCadID;
destination=RVU_bortavaror_air.D_B_TransCadID;
...
RVU_bortavaror_air.percentageTime=zeros(length(origin),1);
for i=1:length(origin)
    ...
    RVU_bortavaror_air.percentageTime(i)=find(sort(airTimeDestinationAlternatives)==airTimeDestination)/length(airTimeDestinationAlternatives);
