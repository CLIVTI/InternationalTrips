...
% Flight
AirImpedansPath='LOS/Air/Impedans.xlsx';
...
% Ferry
FerryImpedansPath='LOS/Ferry/Impedans.xlsx';
...
trainImpedanceLog=trainImpedance;
trainImpedanceLog(2:end,2:end)=log(trainImpedanceLog(2:end,2:end)+0.01);
...
AirImpedans = xlsread(AirImpedansPath) ;
...
for i=1:(size(airCost,1)-1)
    ...
    AirImpedans(i+1,noAirUsedIndex)=nan;
...
AirImpedansLog=AirImpedans;
AirImpedansLog(2:end,2:end)=log(AirImpedansLog(2:end,2:end)+0.01);
...
FerryImpedans = xlsread(FerryImpedansPath) ;
...
for i=1:(size(ferryNTransfer,1)-1)
    ...
    FerryImpedans(i+1,noFerryUsedIndex)=nan;
...
FerryImpedansLog=FerryImpedans;
FerryImpedansLog(2:end,2:end)=log(FerryImpedansLog(2:end,2:end));
...
level_of_service_var_bus.impedansBus=busTime;
level_of_service_var_bus.logimpedansBus=busTimeLog;
...
level_of_service_var_train.impedansTrain=trainImpedance;
level_of_service_var_train.logimpedansTrain=trainImpedanceLog;
...
level_of_service_var_air.impedansAir=AirImpedans;
level_of_service_var_air.logimpedansAir=AirImpedansLog;
...
level_of_service_var_ferry.impedansFerry=FerryImpedans;
level_of_service_var_ferry.logimpedansFerry=FerryImpedansLog;
...
beta_names_fix.(mode_choice_names{2})={'bus_ASC','bus_female','bus_VILLA','bus_age_64'};  
X_names_fix.(mode_choice_names{2})={'ASC','female','VILLA','age_64'};  

beta_names_fix.(mode_choice_names{3})={'train_ASC','train_female','train_VILLA','train_age_64'};   
X_names_fix.(mode_choice_names{3})={'ASC','female','VILLA','age_64'};  

beta_names_fix.(mode_choice_names{4})={'air_ASC','air_female','air_VILLA','air_age_64'};  
X_names_fix.(mode_choice_names{4})={'ASC','female','VILLA','age_64'};  

beta_names_fix.(mode_choice_names{5})={'ferry_ASC','_ferry_female','ferry_VILLA','ferry_age_64'};  
X_names_fix.(mode_choice_names{5})={'ASC','female','VILLA','age_64'};  
