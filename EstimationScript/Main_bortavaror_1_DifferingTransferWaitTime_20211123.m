...
TrainTransferWaitTimePath='LOS/Train/EMMEWeightsAndCarNetworkAsAccessEgress/TransferWaitTime.xlsx';
...
TrainWeightedTotalTime(2:end,2:end)=TrainInVehicleTime(2:end,2:end)+2*TrainAccessEgressTime(2:end,2:end);
TrainWeightedTotalTime_Scenario=TrainInVehicleTime;
TrainWeightedTotalTime_Scenario(2:end,2:end)=TrainInVehicleTime(2:end,2:end)*0.9+2*TrainAccessEgressTime(2:end,2:end);
...
for i=1:(size(trainDistance,1)-1)
    ...
    TrainWeightedTotalTime_Scenario(i+1,noUsedIndex)=nan;
...
%
prediction=Apply_NL_model(final_result_bortavaror_4,...
    RVU_bortavaror,...
    RVU,...
    model_specification_modeChoice,...
    ZoneData,...
...
level_of_service_var_scenario.train.inVehicleTimeBusTrainAir=TrainWeightedTotalTime_Scenario;
prediction_2=Apply_NL_model(final_result_bortavaror_4,...
    RVU_bortavaror,...
    RVU,...
    model_specification_modeChoice,...
    ZoneData,...
...
(nanmean(prediction_2.probability_fullData(:,3))-nanmean(prediction.probability_fullData(:,3)))/nanmean(prediction.probability_fullData(:,3))
