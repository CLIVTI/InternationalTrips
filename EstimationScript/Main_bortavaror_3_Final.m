...
% income
RVU.lowIncome=RVU.HHINK<200000;
RVU.mediumIncome=RVU.HHINK>=200000 & RVU.HHINK<300000;
RVU.highIncome=RVU.HHINK>=300000;
...
RVU_bortavaror=RVU(RVU.bortavaro==3,:);
%% read land use data
...
zonal_data_car.betaNames={'LU_Hotel_beds','LU_Employment'};
zonal_data_car.XNames={'Hotel_beds','Employment'};
...
busCost(2:end,2:end)=busCost(2:end,2:end).*0.18;
...
FerryNumberLinesUsed = xlsread(FerryNumberLinesUsedPath);

% we assume that if there is no ferry line used, the destination is not available, code as nan.
for i=1:(size(FerryNumberLinesUsed,1)-1)
    noFerryUsedIndex=FerryNumberLinesUsed(i+1,:)==0;
...
level_of_service_var_car.travelCost_lowIncome=carCost;
level_of_service_var_car.travelCost_mediumIncome=carCost;
...
level_of_service_var_bus.travelCost_lowIncome=busCost;
level_of_service_var_bus.travelCost_mediumIncome=busCost;
...
level_of_service_var_train.travelCost_lowIncome=trainCost;
level_of_service_var_train.travelCost_mediumIncome=trainCost;
...
level_of_service_var_air.travelCost_lowIncome=airCost;
level_of_service_var_air.travelCost_mediumIncome=airCost;
...
level_of_service_var_ferry.travelCost_lowIncome=ferryCost;
level_of_service_var_ferry.travelCost_mediumIncome=ferryCost;
...
beta_names_fix.(mode_choice_names{2})={'bus_ASC','bus_female','bus_age_17'};  
X_names_fix.(mode_choice_names{2})={'ASC','female','age_17'};  

beta_names_fix.(mode_choice_names{3})={'train_ASC','train_female'};   
X_names_fix.(mode_choice_names{3})={'ASC','female'};  

beta_names_fix.(mode_choice_names{4})={'air_ASC','air_female','air_age_64'};  
X_names_fix.(mode_choice_names{4})={'ASC','female','age_64'};  

beta_names_fix.(mode_choice_names{5})={'ferry_ASC','ferry_female'};  
X_names_fix.(mode_choice_names{5})={'ASC','female'};  
...
% 
final_result_bortavaror_1=NL_model_joint_estimation_log_zonal_flexible(RVU_bortavaror,...
                                                          RVU,...
                                                          model_specification_modeChoice,...
                                                          ZoneData,...
                                                          ZoneID_varname,...
