...
for i=1:(size(airCost,1)-1)
    ...
    AirImpedans(i+1,noUsedIndex)=nan;
...
for i=1:(size(ferryNTransfer,1)-1)
    noUsedIndex=ferryNTransfer(i+1,:)==0 | FerryDistanceFullTrip(i+1,:)<100 ;
    SemesterZonesMatrixFerry(i+1,noUsedIndex)=nan;
    FerryImpedans(i+1,noUsedIndex)=nan;
...
beta_names_fix.(mode_choice_names{3})={'train_ASC','train_age_17','train_age_64'};   
X_names_fix.(mode_choice_names{3})={'ASC','age_17','age_64'};  
