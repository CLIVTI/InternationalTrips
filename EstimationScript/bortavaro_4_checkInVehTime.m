...
AirTotalTime(2:end,2:end)=AirInVehicleTime(2:end,2:end)+1.36.*AirAccessEgressTime(2:end,2:end);
...
for i=1:(size(airCost,1)-1)
    ...
    AirTotalTime(i+1,noUsedIndex)=nan;
...
% level_of_service_var_air.inVehicleTimeBusTrainAirFerry=AirAccessEgressTime;
...
level_of_service_var_air.inVehicleTimeBusTrainAirFerry=AirTotalTime;
...
InVehicleTime=0;
 for i=1:size(RVU_bortavaror,1)
     if RVU_bortavaror.Mode(i)==5
         disp(InVehicleTimeData(InVehicleTimeStartZone==startZoneID(i),InVehicleTimeEndZone==endZoneID(i)));
     end
